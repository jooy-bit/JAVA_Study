...
public class B07CinstructorExam {
    public static void main(String[] args) {
        //문자열 리터럴 message 변수로 참조
        String message="Hello!"; //불변 객체
        String temp = "Hello!"; //불변 객체
        //문자열 객체 생성하기
    //1> 메소드 오버로딩 : 메소드 이름 같으면서 리턴타입, 인자 수와 형식이 다른 정의(?)
        String str1 = new String();                                   //생성자 인자 없음
        String str2 = new String("Hello!");                  //생성자 인자가 문자열 리터럴
        String str3 = new String(new char[]{'H','e','l','l','o','!'});//생성자 인자가 문자배열
        System.out.println("new 연산으로 생성된 String 객체 출력해보기");
        System.out.println("\tstr1"+ str1);     //길이기 0인 문자열 객체
        System.out.println("\tstr2"+ str2);     //
        System.out.println("\tstr3"+ str3);     //

    //2> 문자열 리터럴의 특징 : 문자열 내용이 같으면 또 메모리에 새로 저장하지 않고 
        ...
        System.out.println("message==temp ?  " + (message==str2)); //단, new 연산으로 생성된 객체는 언제나 메모리 새로 할당     //false
    ...
    //3> 문자열 내용 비교 : equals 메소드 사용
        ...
        int num = sc.nextInt();             // 입력값을 정수형식으로 변화
        double pi =sc.nextDouble();         //         실수 형식으로 변환
        sc.nextLine(); //키보드 입력을 저장하는 버퍼에서 실수 입력 숫자 뒤에 엔터 친 것이 남아있으므로 처리하기 떄문에
        String name = sc.nextLine();        //         문자열로 변환

        System.out.println(num+pi+name);
    
        System.out.println( "위의 입력테스트에서 nexInt 또는 nextDouble 은 엔터처리를 안하므로 다음에 나오는 nextline 으로 자동 저장");
        ...
        temp =sc.nextLine();                    //문자열을 double로 변환하는 메소드 사용
        pi = Double.parseDouble(temp);          //숫자와 . 만 입력하세요
